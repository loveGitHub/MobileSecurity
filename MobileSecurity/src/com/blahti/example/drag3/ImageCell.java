	...
	public void onDropCompleted(View target, boolean success) {
		...
		if (success) {
			...
			if (mCellNumber >= 0) {
				...
				// setImageDrawable(null); // Culprit Line for us
			} else {
 ...
 */
	...
	public void onDrop(DragSource source, int x, int y, int xOffset,
		...
		Drawable tempd = getDrawable();

		Log.d("Drawable ???????????>>>>>>>>> ", "" + d.toString());
		...
		if (d != null) {
			...
			if (tempd != null) {
				...
				sourceView.setImageDrawable(tempd);
